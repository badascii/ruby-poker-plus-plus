= Extension of Rob Olson's ruby-poker gem for Portland Code School

  - Added spec files for both poker_hand.rb and card.rb
  - Added RakeTask which runs spec files using 'rake test'
  - Extended original gem with a new method that mocks the user if they miss a
    royal flush by 1 card
  - Added tests that check validity of new method
